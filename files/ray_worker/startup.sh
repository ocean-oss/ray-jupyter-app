...
start.sh ray start \
  --address {{ engines.ray_jupyter.vars.ray_head_host }}:{{ engines.ray_jupyter.vars.ray_head_port }} \
  --node-ip-address $(hostname -I) \
  --block \
  --object-manager-port {{ engines.ray_worker.vars.ray_object_manager_port }} \
  --node-manager-port {{ engines.ray_worker.vars.ray_node_manager_port }} \
